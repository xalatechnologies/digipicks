 ...
 * infrastructure in @digipicks/app-shell or @digipicks/sdk.
 *
 ...
 *
 * Allowed sources:
 *   @digipicks/app-shell  — shared AuthProvider, useAuth, ProtectedRoute
 *   @digipicks/sdk        — useAuth re-export from SDK layer
 *
...
const ALLOWED_AUTH_SOURCES = ['@digipicks/app-shell', '@digipicks/sdk'];
...
export default {
  meta: {
    ...
    docs: {
      description:
        ...
        'Import from @digipicks/app-shell or @digipicks/sdk instead.',
      category: 'Security',
      recommended: true,
    },
    messages: {
      noAppLocalAuth:
        ...
        'Use @digipicks/app-shell or @digipicks/sdk instead.',
    },
    schema: [],
  },
  ...
  create(context) {
    ...
    if (filename.includes('/packages/app-shell/') || filename.includes('/packages/sdk/')) {
      return {};
    }
    ...
    return {
      ImportDeclaration(node) {
        ...
        const isAllowed = ALLOWED_AUTH_SOURCES.some((pkg) => source === pkg || source.startsWith(pkg + '/'));
        if (isAllowed) return;
