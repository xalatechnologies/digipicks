...
This dictionary maps raw HTML elements and common UI patterns to their `@digipicks/ds` design system equivalents.
...
```
Use <Button> from @digipicks/ds instead of <button>.
Use <Textfield>, <Checkbox>, or <Radio> from @digipicks/ds instead of <input>.
Use <Select> or <NativeSelect> from @digipicks/ds instead of <select>.
Use <Heading level={N}> from @digipicks/ds instead of <hN>.
Use <Paragraph> from @digipicks/ds instead of <p>.
Use <DataTable> or DS Table from @digipicks/ds instead of <table>.
```
