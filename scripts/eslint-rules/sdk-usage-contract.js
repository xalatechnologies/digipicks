 ...
 *
 * Package: @digipicks/sdk (the actual SDK used across apps)
 */
...
export default {
  meta: {
    ...
    messages: {
      forbiddenImport: "App '{{app}}' cannot import from SDK namespace '{{namespace}}'. Allowed: {{allowed}}",
    },
    schema: [],
  },
  ...
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        ...
        // Check @digipicks/sdk imports
        if (!source.startsWith('@digipicks/sdk')) {
          return;
        }

        // Extract namespace from import path
        // e.g., '@digipicks/sdk/tenant' -> 'tenant'
        const parts = source.replace('@digipicks/sdk/', '').split('/');
