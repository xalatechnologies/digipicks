...
export function useCreatorApplications(
    ...
    const data = useConvexQuery(
        api.domain.creatorApplication.list,
...
export function useCreatorApplication(id: string | undefined) {
    const data = useConvexQuery(
        api.domain.creatorApplication.get,
        id ? { id } : "skip"
    );

    const isLoading = id !== undefined && data === undefined;
    const application: CreatorApplication | null = data
...
export function useRequestMoreInfoCreatorApplication() {
    const mutationFn = useConvexMutation(
        api.domain.creatorApplication.requestMoreInfo
    );
    return {
        mutate: mutationFn,
        mutateAsync: mutationFn,
...
export function useResubmitCreatorApplication() {
    const mutationFn = useConvexMutation(
        api.domain.creatorApplication.resubmit
    );
    return {
        mutate: mutationFn,
        mutateAsync: mutationFn,
