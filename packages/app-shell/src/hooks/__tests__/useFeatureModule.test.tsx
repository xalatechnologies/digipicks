...
vi.mock("@digilist-saas/sdk", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@digilist-saas/sdk")>();
  return {
    ...actual,
    useSessionTenantId: (...args: unknown[]) =>
      mockUseSessionTenantId(...args),
    useTenantFeatureFlags: (...args: unknown[]) =>
      mockUseTenantFeatureFlags(...args),
  };
});
...
describe("FeatureGate", () => {
  ...
  it("renders fallback when module is disabled", () => {
    ...
    render(
      <FeatureGate
        module="reviews"
        fallback={<span data-testid="fallback">Not available</span>}
      >
