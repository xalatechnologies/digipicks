...
import { ErrorBoundary, DesignsystemetProvider } from '@digipicks/ds';
import { i18n, getDirection } from '@digipicks/i18n';
import type { Locale as SharedLocale, Direction, AppId } from '@digipicks/shared';
import type { Locale as I18nLocale } from '@digipicks/i18n';
...
// =============================================================================
// LocaleProvider - Manages locale state (i18n is initialized synchronously in @digipicks/i18n)
// =============================================================================
...
function RuntimeProvider({
  ...
  const value: RuntimeContext = useMemo(() => ({ appId, env }), [appId, env]);
...
function ThemedApp({
  ...
  return (
    <DesignsystemetProvider theme="digilist" colorScheme={colorScheme} size="md" typography="primary">
      {children}
          ...
          <LocaleProvider defaultLocale={defaultLocale} supportedLocales={supportedLocales}>
            <LocaleContext.Consumer>
              {(localeCtx: LocaleContextValue | null) => (
                <ThemedApp locale={localeCtx?.locale ?? defaultLocale} direction={localeCtx?.direction ?? 'ltr'}>
                  <RuntimeProvider appId={appId} env={{ appId }}>
                    {children}
                  </RuntimeProvider>
                </ThemedApp>
              )}
