...
1. **Before adding UI:** Query `@digilist-saas/ds/registry` for components. Use `resolveNeed()`, `components`, or `getComponent()`.
2. **Before implementing auth/realtime:** Check `PROHIBITIONS` and `REQUIREMENTS` in gatekeeper. Use `@digilist-saas/app-shell` only.
...
2. **Building a new app?** See `docs/planning/CLI_APP_GENERATOR_PLAN.md` and provider stack in `@digilist-saas/ds/registry`.
3. **Adding a component?** Check if it exists: `import { components } from '@digilist-saas/ds/registry'`.
...
| ID | Rule | Use Instead |
|----|------|-------------|
| `no-app-local-auth` | No app-local AuthProvider, useAuth | `@digilist-saas/app-shell` |
| `no-app-local-realtime` | No custom realtime providers | `RealtimeProvider` / `ConvexRealtimeProvider` from app-shell |
| `no-direct-digdir` | No `@digdir/*` imports in apps | `@digilist-saas/ds` |
| `no-raw-html` | No raw `<button>`, `<input>`, `<table>` for UI | Button, Textfield, DataTable from `@digilist-saas/ds` |
| `no-inline-svg` | No inline `<svg>` | Icon components from `@digilist-saas/ds` |
...
| ID | Rule |
|----|------|
| `import-ui-from-ds` | Import UI from `@digilist-saas/ds` |
| `import-styles-once` | Import `@digilist-saas/ds/styles` once in main.tsx |
| `use-theme-base` | Import `@digilist-saas/ds/platform-base` |
...
| Button | `<Button>` from `@digilist-saas/ds` |
...
| Data table | `<DataTable>` from `@digilist-saas/ds` |
...
import { providers as providerModule } from '@digilist-saas/ds/registry';

// Resolve "I need a button" → { component: 'button', source: '@digilist-saas/ds' }
