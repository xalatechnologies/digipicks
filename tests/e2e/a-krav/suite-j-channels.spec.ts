...
test.describe("Suite J: Channels", () => {
  ...
  test("CHN-001: Web purchase channel tag", async ({
    page,
    loginAsUser,
  }, testInfo) => {
  ...
  test("CHN-002: Counter purchase channel tag", async ({
    page,
    loginAsCounter,
  }, testInfo) => {
    ...
    const counterBtn = page
      .locator(
        'button:has-text("Nytt salg"), button:has-text("Ny ordre"), button:has-text("Kassesalg")'
      )
      .first();
