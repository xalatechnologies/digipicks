...
describe('creatorApplication/mutations — submit', () => {
  it('creates a new application with pending status', async () => {
    ...
    const app = (await t.run(async (ctx) => ctx.db.get(result.id as any))) as any;
  ...
  it('rejects if user already has a pending application', async () => {
    ...
    await expect(submitApplication(t)).rejects.toThrow('You already have a pending application');
  });
  ...
  it('rejects if user is already an approved creator', async () => {
    ...
    await expect(submitApplication(t)).rejects.toThrow('You are already an approved creator');
  });
  ...
  it('allows submission after rejection', async () => {
    ...
    const app = (await t.run(async (ctx) => ctx.db.get(second.id as any))) as any;
...
describe('creatorApplication/mutations — approve', () => {
  it('approves a pending application', async () => {
    ...
    const app = (await t.run(async (ctx) => ctx.db.get(id as any))) as any;
  ...
  it('rejects approve on already approved application', async () => {
    ...
    await expect(t.mutation(api.functions.approve, { id, reviewedBy: ADMIN })).rejects.toThrow(
...
describe('creatorApplication/mutations — reject', () => {
  ...
  it('rejects an already approved application throws', async () => {
    ...
    await expect(t.mutation(api.functions.reject, { id, reviewedBy: ADMIN })).rejects.toThrow(
      'Cannot reject application with status "approved"',
    );
  });
});
...
describe('creatorApplication/mutations — requestMoreInfo', () => {
  it('transitions pending to more_info_requested', async () => {
    ...
    expect(app?.reviewNote).toBe('Please provide your Action Network profile');
  });
  ...
  it('rejects if application not pending', async () => {
    ...
    ).rejects.toThrow('Cannot request more info for application with status "approved"');
  });
});
...
describe('creatorApplication/mutations — resubmit', () => {
  ...
  it('rejects resubmit on pending application', async () => {
    ...
    ).rejects.toThrow('Cannot resubmit application with status "pending"');
  });
});
