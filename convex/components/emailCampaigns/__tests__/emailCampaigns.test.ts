...
describe('create', () => {
  ...
  it('rejects invalid campaign type', async () => {
    const t = convexTest(schema, modules);
    await expect(createCampaign(t, { campaignType: 'invalid' })).rejects.toThrow('Invalid campaign type');
  });

  it('rejects invalid segment type', async () => {
    const t = convexTest(schema, modules);
    await expect(createCampaign(t, { segment: { type: 'nonexistent' } })).rejects.toThrow('Invalid segment type');
  });

  it('rejects empty name', async () => {
    const t = convexTest(schema, modules);
    await expect(createCampaign(t, { name: '  ' })).rejects.toThrow('Campaign name cannot be empty');
  });

  it('rejects empty subject', async () => {
    const t = convexTest(schema, modules);
    await expect(createCampaign(t, { subject: '' })).rejects.toThrow('Campaign subject cannot be empty');
  });
...
describe('cancel', () => {
  ...
  it('rejects cancelling sent campaigns', async () => {
    ...
    await expect(t.mutation(api.functions.cancel, { id: id as any })).rejects.toThrow(
...
describe('markSending / markSent', () => {
  ...
  it('rejects sending already-sent campaigns', async () => {
    ...
    await expect(t.mutation(api.functions.markSending, { id: id as any })).rejects.toThrow(
      'Can only start sending draft or scheduled campaigns',
    );
  });
});
