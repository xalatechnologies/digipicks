        ...
        if (status) {
            ...
            campaigns = await ctx.db
                ...
                .withIndex("by_tenant_type", (q) =>
                    q.eq("tenantId", tenantId).eq("campaignType", campaignType)
                )
                .collect();
        } else {
            campaigns = await ctx.db
...
export const listScheduledReady = query({
    ...
    handler: async (ctx, { now }) => {
        ...
        return campaigns.filter(
            (c) => c.scheduledAt !== undefined && c.scheduledAt <= now
        );
    },
});
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!VALID_SEGMENT_TYPES.includes(args.segment.type)) {
            throw new Error(
                `Invalid segment type: ${args.segment.type}. Must be one of: ${VALID_SEGMENT_TYPES.join(", ")}`
            );
        }
...
export const updateRecipientStatus = mutation({
    ...
    handler: async (ctx, { id, status, resendId, bounceReason }) => {
        ...
        const campaign = await ctx.db.get(recipient.campaignId as any);
        if (campaign) {
            const counterPatch: Record<string, any> = {};
            if (status === "sent")
                counterPatch.sentCount = campaign.sentCount + 1;
            if (status === "opened")
                counterPatch.openCount = campaign.openCount + 1;
            if (status === "clicked")
                counterPatch.clickCount = campaign.clickCount + 1;
            if (status === "bounced")
                counterPatch.bounceCount = campaign.bounceCount + 1;
            if (status === "unsubscribed")
                counterPatch.unsubscribeCount = campaign.unsubscribeCount + 1;
