...
export const moderate = mutation({
    ...
    handler: async (ctx, { id, moderationStatus, moderatedBy, moderationNote }) => {
        ...
        await ctx.db.patch(id, {
            ...
            ...(moderationStatus === "rejected" || moderationStatus === "hidden"
                ? { status: "archived" }
                : {}),
        });
    ...
    handler: async (ctx, { tenantId, moderationStatus, creatorId, limit }) => {
        ...
        if (moderationStatus) {
            ...
            const flagged = await ctx.db
                .query("picks")
                .withIndex("by_tenant_moderationStatus", (q) =>
                    q.eq("tenantId", tenantId).eq("moderationStatus", "flagged")
                )
                .collect();
...
export const moderationStats = query({
    ...
    handler: async (ctx, { tenantId }) => {
        ...
        const rejected = await ctx.db
            .query("picks")
            .withIndex("by_tenant_moderationStatus", (q) =>
                q.eq("tenantId", tenantId).eq("moderationStatus", "rejected")
            )
            .collect();
        const hidden = await ctx.db
            .query("picks")
            .withIndex("by_tenant_moderationStatus", (q) =>
                q.eq("tenantId", tenantId).eq("moderationStatus", "hidden")
            )
            .collect();
        const approved = await ctx.db
            .query("picks")
            .withIndex("by_tenant_moderationStatus", (q) =>
                q.eq("tenantId", tenantId).eq("moderationStatus", "approved")
            )
            .collect();
