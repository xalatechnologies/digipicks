...
describe('picks/mutations — update', () => {
  ...
  it('throws when editing odds/units/selection on a graded pick', async () => {
    ...
    await expect(t.mutation(api.functions.update, { id: id as any, oddsAmerican: '+200' })).rejects.toThrow(
  ...
  it('throws for invalid sport on update', async () => {
    ...
    await expect(t.mutation(api.functions.update, { id: id as any, sport: 'Invalid' })).rejects.toThrow(
      'Invalid sport',
    );
  });
});
...
describe('picks/mutations — grade', () => {
  ...
  it('throws when grading an already graded pick', async () => {
    ...
    await expect(t.mutation(api.functions.grade, { id: id as any, result: 'lost', gradedBy: GRADER })).rejects.toThrow(
      'Pick already graded',
    );
  });
});
...
describe('picks/mutations — remove', () => {
  ...
  it('throws for non-existent pick', async () => {
    ...
    const { id } = await createPick(t);
    await t.mutation(api.functions.remove, { id: id as any });
    await expect(t.mutation(api.functions.remove, { id: id as any })).rejects.toThrow('Pick not found');
  });
});
...
describe('picks/queries — get', () => {
  ...
  it('throws for non-existent pick', async () => {
    ...
    const { id } = await createPick(t);
    await t.mutation(api.functions.remove, { id: id as any });
    await expect(t.query(api.functions.get, { id: id as any })).rejects.toThrow('Pick not found');
  });
});
