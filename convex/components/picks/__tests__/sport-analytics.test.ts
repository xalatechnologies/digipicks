...
async function gradePick(t: ReturnType<typeof convexTest>, id: string, result: 'won' | 'lost' | 'push' | 'void') {
...
describe('picks/queries — sportDashboard', () => {
  ...
  it('calculates aggregate stats for a sport', async () => {
    ...
    const { id: w2 } = await createPick(t, {
      sport: 'NBA',
      creatorId: CREATOR_B,
      event: 'W2',
      oddsDecimal: 1.5,
  ...
  it('returns top creators sorted by ROI', async () => {
    ...
    const { id: a1 } = await createPick(t, {
      sport: 'NBA',
      creatorId: CREATOR_A,
      event: 'A1',
      oddsDecimal: 3.0,
    ...
    const { id: b1 } = await createPick(t, {
      sport: 'NBA',
      creatorId: CREATOR_B,
      event: 'B1',
      oddsDecimal: 1.5,
...
describe('picks/queries — creatorStatsBySport', () => {
  ...
  it('breaks down stats by sport', async () => {
    ...
    // NBA: 2W 1L
    const { id: nba1 } = await createPick(t, {
      sport: 'NBA',
      creatorId: CREATOR_A,
      oddsDecimal: 2.0,
      units: 1,
    ...
    await gradePick(t, nba1, 'won');
    const { id: nba2 } = await createPick(t, {
      sport: 'NBA',
      creatorId: CREATOR_A,
      oddsDecimal: 2.0,
      units: 1,
    ...
    const { id: nba3 } = await createPick(t, {
      sport: 'NBA',
      creatorId: CREATOR_A,
      oddsDecimal: 1.91,
      units: 1,
    ...
    // NFL: 1W
    const { id: nfl1 } = await createPick(t, {
      sport: 'NFL',
      creatorId: CREATOR_A,
      oddsDecimal: 1.5,
      units: 2,
  ...
  it('calculates ROI per sport', async () => {
    ...
    const { id: nba } = await createPick(t, {
      sport: 'NBA',
      creatorId: CREATOR_A,
      oddsDecimal: 3.0,
      units: 1,
    ...
    const { id: nfl } = await createPick(t, {
      sport: 'NFL',
      creatorId: CREATOR_A,
      oddsDecimal: 1.91,
      units: 1,
