...
export const createPost = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!VALID_CONTENT_FORMATS.includes(contentFormat)) {
      throw new Error(`Invalid content format: ${contentFormat}. Must be one of: ${VALID_CONTENT_FORMATS.join(', ')}`);
    }

    const accessLevel = args.accessLevel ?? 'free';
    if (!VALID_ACCESS_LEVELS.includes(accessLevel)) {
      throw new Error(`Invalid access level: ${accessLevel}. Must be one of: ${VALID_ACCESS_LEVELS.join(', ')}`);
    }
  ...
  handler: async (ctx, args) => {
    ...
    if (args.accessLevel !== undefined) {
      if (!VALID_ACCESS_LEVELS.includes(args.accessLevel)) {
        throw new Error(`Invalid access level: ${args.accessLevel}. Must be one of: ${VALID_ACCESS_LEVELS.join(', ')}`);
      }
