...
async function shouldNotify(
  ...
  try {
    const prefs = (await ctx.runQuery(components.notifications.functions.getPreferences, { userId })) as Array<{
      channel: string;
      category: string;
      enabled: boolean;
    }>;
    const match = prefs.find((p) => p.channel === 'in_app' && p.category === category);
    ...
    for (const event of pending) {
      try {
        ...
        if (event.topic === 'resources.resource.created') {
          ...
          const review = payload.reviewId ? await safeGetReview(ctx, payload.reviewId as string) : null;
          ...
          if (reviewAuthor) {
            ...
            await ctx.runMutation(components.notifications.functions.create, {
              ...
              body: isApproved ? 'Din anmeldelse er publisert.' : 'Din anmeldelse ble ikke godkjent.',
          ...
          if (payload.creatorId) {
            const subscribers = await ctx.runQuery(components.subscriptions.functions.listCreatorSubscribers, {
          ...
          for (const collab of collaborators) {
            ...
            const subscribers = await ctx.runQuery(components.subscriptions.functions.listCreatorSubscribers, {
              creatorId: collab.creatorId as string,
              status: 'active',
            });
            for (const sub of subscribers as any[]) {
              if (
                sub.userId &&
                !notifiedUsers.has(sub.userId as string) &&
                (await shouldNotify(ctx, sub.userId as string, 'picks'))
              ) {
...
export const listRecent = internalQuery({
  ...
  handler: async (ctx, { tenantId, topic, status, limit = 50 }) => {
    ...
    if (status) {
      ...
      events = await ctx.db.query('outboxEvents').order('desc').take(limit);
    }
