...
describe('RATE_LIMITS', () => {
  ...
  describe('token bucket limits', () => {
    const tokenBucketLimits = Object.entries(RATE_LIMITS).filter(([, config]) => config.kind === 'token bucket');
...
describe('rateLimitKeys', () => {
  ...
  it('tenantUser builds "tenant:{tenantId}:user:{userId}" format', () => {
    expect(rateLimitKeys.tenantUser('t123', 'u456')).toBe('tenant:t123:user:u456');
  });
