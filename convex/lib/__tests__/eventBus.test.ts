...
describe('eventBus — emit', () => {
  ...
  it('events are scoped by tenantId — different tenants are independent', async () => {
    ...
    const all = await t.run(async (ctx) => ctx.db.query('outboxEvents').collect());
...
describe('eventBus — processEvents', () => {
  it('marks a pending event as processed', async () => {
    ...
    await t.run(async (ctx) => ctx.runMutation(internal.lib.eventBus.processEvents, { batchSize: 10 }));
  ...
  it('respects batchSize — only processes N events per run', async () => {
    ...
    // Process 3 at a time
    const result = await t.run(async (ctx) => ctx.runMutation(internal.lib.eventBus.processEvents, { batchSize: 3 }));
  ...
  it('default batchSize is 50 (no arg needed)', async () => {
    ...
    const result = await t.run(async (ctx) => ctx.runMutation(internal.lib.eventBus.processEvents, {}));
...
describe('eventBus — retryDeadLetter', () => {
  ...
  it('throws when event does not exist', async () => {
    ...
    await expect(
      t.run(async (ctx) => ctx.runMutation(internal.lib.eventBus.retryDeadLetter, { eventId: fakeId as any })),
  ...
  it('throws when event is not in dead_letter or failed status', async () => {
    ...
    await expect(
      t.run(async (ctx) => ctx.runMutation(internal.lib.eventBus.retryDeadLetter, { eventId: eventId as any })),
...
describe('eventBus — listRecent', () => {
  it('returns all recent events (default limit 50)', async () => {
    ...
    const events = await t.run(async (ctx) => ctx.runQuery(internal.lib.eventBus.listRecent, {}));
  ...
  it('filters by status when supplied', async () => {
    ...
    const pending = await t.run(async (ctx) => ctx.runQuery(internal.lib.eventBus.listRecent, { status: 'pending' }));
  ...
  it('filters by tenantId when supplied', async () => {
    ...
    const forTenant = await t.run(async (ctx) => ctx.runQuery(internal.lib.eventBus.listRecent, { tenantId: TENANT }));
  ...
  it('respects limit parameter', async () => {
    ...
    const limited = await t.run(async (ctx) => ctx.runQuery(internal.lib.eventBus.listRecent, { limit: 2 }));
...
describe('eventBus — cleanupProcessed', () => {
  it('deletes processed events older than the cutoff', async () => {
    ...
    const remaining = await t.run(async (ctx) => ctx.db.query('outboxEvents').collect());
  ...
  it('does not delete pending or dead_letter events — only processed', async () => {
    ...
    const result = await t.run(async (ctx) => ctx.runMutation(internal.lib.eventBus.cleanupProcessed, {}));
...
describe('outboxEvents schema — index correctness', () => {
  ...
  it('by_topic index allows querying by topic + status', async () => {
    ...
    const bookingCreatedPending = await t.run(async (ctx) =>
      ctx.db
        .query('outboxEvents')
        .withIndex('by_topic', (q) => q.eq('topic', 'bookings.booking.created').eq('status', 'pending'))
        .collect(),
    );
  ...
  it('by_tenant index allows querying events for a specific tenant', async () => {
    ...
    const tenantEvents = await t.run(async (ctx) =>
      ctx.db
        .query('outboxEvents')
        .withIndex('by_tenant', (q) => q.eq('tenantId', TENANT).eq('status', 'pending'))
        .collect(),
    );
  ...
  it('by_created index is queryable without errors', async () => {
    ...
    const events = await t.run(async (ctx) => ctx.db.query('outboxEvents').withIndex('by_created').collect());
...
describe('eventBus — exponential backoff', () => {
  ...
  it('processEvents sets lastAttemptAt on processing start', async () => {
    ...
    await t.run(async (ctx) => ctx.runMutation(internal.lib.eventBus.processEvents, {}));
...
describe('eventBus — getQueueStats', () => {
  it('returns counts by status and health indicator', async () => {
    ...
    const stats = await t.run(async (ctx) => ctx.runQuery(internal.lib.eventBus.getQueueStats, {}));
  ...
  it('filters by tenantId when provided', async () => {
    ...
    const stats = await t.run(async (ctx) => ctx.runQuery(internal.lib.eventBus.getQueueStats, { tenantId: TENANT }));
...
describe('eventBus — retryAllDeadLetters', () => {
  it('resets all dead letter events to pending', async () => {
    ...
    const result = await t.run(async (ctx) => ctx.runMutation(internal.lib.eventBus.retryAllDeadLetters, {}));
