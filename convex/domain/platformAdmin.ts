...
export const platformStats = query({
  ...
  handler: async (ctx) => {
    ...
    const pendingTenants = allTenants.filter((t: any) => t.status === 'pending').length;
...
export const listAllUsers = query({
  ...
  handler: async (ctx, { status, role }) => {
    ...
    const tenantIds = [...new Set(users.map((u: any) => u.tenantId).filter(Boolean) as string[])];
    ...
    return users.map((u: any) => ({
      ...
      lastActive: u.lastLoginAt
        ...
        : u._creationTime
          ...
          : '–',
      createdAt: u._creationTime ? new Date(u._creationTime).toISOString().split('T')[0] : '–',
    }));
  },
});
...
export const recentActivity = query({
  ...
  handler: async (ctx, { limit }) => {
    ...
    for (const tenant of allTenants) {
      try {
        const entries = await ctx.runQuery(components.audit.functions.listForTenant, {
    ...
    allEntries.sort((a, b) => (b._creationTime || 0) - (a._creationTime || 0));
      ...
      if (updates.status === 'suspended' || updates.status === 'inactive') {
        ...
        if (email) {
          ...
          const html = wrapInEmailLayout({
            ...
            tenantName: process.env.PLATFORM_NAME || 'Xala',
          });
 ...
 */
async function cascadeUserStatusToTenants(
  ctx: any,
  userId: any,
  newTenantStatus: string,
  listingAction: 'pause' | 'unpause' | 'deactivate',
) {
...
export const updateTenant = mutation({
  args: {
    ...
    status: v.optional(
      v.union(v.literal('active'), v.literal('suspended'), v.literal('pending'), v.literal('deleted')),
    ),
