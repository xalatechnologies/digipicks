...
async function hasSubscriptionAccess(
  ...
  if (creatorId) {
    const membership = await ctx.runQuery(components.subscriptions.functions.getMembershipByUserAndCreator, {
      userId: viewerId,
      creatorId,
    });
  ...
  const membership = await ctx.runQuery(components.subscriptions.functions.getMembershipByUser, { userId: viewerId });
  return membershipGrantsAccess(membership);
}
 ...
 */
async function hasCoPostAccess(ctx: QueryCtx, viewerId: string | undefined, pickId: string): Promise<boolean> {
  if (!viewerId) return true;

  const collaborators = await ctx.runQuery(components.picks.functions.listPickCollaborators, { pickId });
  if ((collaborators as any[]).length === 0) return false;

  const creatorIds = (collaborators as any[]).map((c: any) => c.creatorId);
  const memberships = await ctx.runQuery(components.subscriptions.functions.listMembershipsByCreatorIds, {
    userId: viewerId,
    creatorIds,
  });
 ...
 */
async function hasPickAccess(ctx: QueryCtx, viewerId: string | undefined, pick: any): Promise<boolean> {
  if (!viewerId) return true;
...
export const list = query({
  ...
  handler: async (ctx, { tenantId, creatorId, sport, result, status, limit, viewerId }) => {
    ...
    for (const pick of picks as any[]) {
      ...
      const collaborators = await ctx.runQuery(components.picks.functions.listPickCollaborators, {
        pickId: pick._id as string,
      });
      const enriched = {
        ...pick,
        creator: user ? { id: user._id, name: user.name, email: user.email, displayName: user.displayName } : null,
...
export const get = query({
  ...
  handler: async (ctx, { id, viewerId }) => {
    ...
    const collaborators = await ctx.runQuery(components.picks.functions.listPickCollaborators, { pickId: id });
...
async function batchCreatorVerification(
  ...
  const apps = await Promise.all(
    creatorIds.map((id) =>
      ctx
        .runQuery(components.creatorApplication.functions.getByUser, {
          tenantId,
          userId: id,
        })
        .catch(() => null),
...
export const discoverCreators = query({
  ...
  handler: async (ctx, { tenantId, search, sport }) => {
    ...
    const brandConfigs = await ctx.runQuery(components.tenantConfig.queries.listCreatorBrandConfigs, {
      tenantId: tenantId as string,
    });
    const brandMap = new Map((brandConfigs as any[]).map((b: any) => [b.creatorId, b]));
...
export const feedFollowing = query({
  ...
  handler: async (ctx, { tenantId, userId, sport, result, limit, cursor }) => {
    ...
    const memberships = await ctx.runQuery(components.subscriptions.functions.listMemberships, {
      tenantId: tenantId as string,
      userId,
      status: 'active',
    });
    ...
    const users = await Promise.all(creatorIdSet.map((id: string) => ctx.db.get(id as Id<'users'>).catch(() => null)));
    const userMap = new Map(users.filter(Boolean).map((u: any) => [u!._id, u]));
  ...
  handler: async (ctx, { tenantId, userId, sport, result, limit, cursor }) => {
    ...
    if (userId) {
      ...
      subscribedCreatorIds = new Set(
        (memberships as any[]).filter((m: any) => m.creatorId).map((m: any) => m.creatorId as string),
      );
    }
    ...
    for (const pick of picks as any[]) {
      ...
      let isUnlocked = !!(userId && (pick.creatorId === userId || subscribedCreatorIds.has(pick.creatorId)));
...
export const moderatePick = mutation({
  ...
  handler: async (ctx, { id, moderatedBy, moderationStatus, moderationNote }) => {
    ...
    if (creatorId && ['rejected', 'hidden', 'approved'].includes(moderationStatus)) {
      const statusLabel =
        moderationStatus === 'approved' ? 'approved' : moderationStatus === 'rejected' ? 'rejected' : 'hidden';
...
export const listPickReports = query({
  ...
  handler: async (ctx, { pickId, callerId, status }) => {
    ...
    const users = await Promise.all(reporterIds.map((id: string) => ctx.db.get(id as Id<'users'>).catch(() => null)));
    const userMap = new Map(users.filter(Boolean).map((u: any) => [u!._id, u]));
    ...
    return (reports as any[]).map((report: any) => {
      ...
      return {
        ...report,
        reporter: user ? { id: user._id, name: user.name, email: user.email, displayName: user.displayName } : null,
      };
    });
  },
});
