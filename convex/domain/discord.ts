...
export const getConnection = query({
  ...
  handler: async (ctx, { tenantId, userId }) => {
    ...
    const connection = await ctx.runQuery(components.discord.functions.getConnection, { tenantId, userId });
...
export const getServerConfig = query({
  ...
  handler: async (ctx, { tenantId, creatorId }) => {
    const config = await ctx.runQuery(components.discord.functions.getServerConfig, { tenantId, creatorId });
...
export const listRoleMappings = query({
  ...
  handler: async (ctx, { tenantId, creatorId }) => {
    const mappings = await ctx.runQuery(components.discord.functions.listRoleMappings, { tenantId, creatorId });
...
export const listSyncLog = query({
  ...
  handler: async (ctx, { tenantId, userId }) => {
    return ctx.runQuery(components.discord.functions.listSyncLog, { tenantId, userId });
  },
});
...
export const saveConnection = mutation({
  ...
  handler: async (ctx, args) => {
    const result = await ctx.runMutation(components.discord.functions.upsertConnection, {
    ...
    await emitEvent(ctx, 'discord.connection.created', args.tenantId as string, 'discord', {
      userId: args.userId as string,
      discordUserId: args.discordUserId,
...
export const disconnectUser = mutation({
  ...
  handler: async (ctx, args) => {
    const result = await ctx.runMutation(components.discord.functions.disconnectUser, {
      tenantId: args.tenantId as string,
      userId: args.userId as string,
    });
    ...
    await emitEvent(ctx, 'discord.connection.revoked', args.tenantId as string, 'discord', {
      userId: args.userId as string,
    });
...
export const configureServer = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.runMutation(components.discord.functions.upsertServerConfig, {
      tenantId: args.tenantId as string,
...
export const setRoleMapping = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.runMutation(components.discord.functions.upsertRoleMapping, {
      tenantId: args.tenantId as string,
      creatorId: args.creatorId as string,
      tierId: args.tierId,
...
export const removeRoleMapping = mutation({
  ...
  handler: async (ctx, args) => {
    const result = await ctx.runMutation(components.discord.functions.removeRoleMapping, {
      tenantId: args.tenantId as string,
      tierId: args.tierId,
    });
...
export const syncRole = action({
  ...
  handler: async (ctx, args) => {
    ...
    const connection: any = await ctx.runQuery(components.discord.functions.getConnection, {
    ...
    const serverConfig: any = await ctx.runQuery(components.discord.functions.getServerConfig, {
      tenantId: args.tenantId,
    ...
    const roleMapping: any = await ctx.runQuery(components.discord.functions.getRoleMappingByTier, {
      tenantId: args.tenantId,
      tierId: args.tierId,
    });
    ...
    const logEntry: any = await ctx.runMutation(components.discord.functions.createSyncLogEntry, {
      tenantId: args.tenantId,
      userId: args.userId,
      discordUserId: connection.discordUserId,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        await ctx.runMutation(components.discord.functions.updateSyncLogStatus, {
      ...
      // Emit event
      const eventTopic = args.action === 'assign' ? 'discord.role.assigned' : 'discord.role.removed';
...
export const handleSubscriptionEvent = internalMutation({
  ...
  handler: async (ctx, { tenantId, topic, payload }) => {
    ...
    if (topic === 'subscriptions.membership.created' || topic === 'subscriptions.membership.upgraded') {
      roleAction = 'assign';
    } else if (topic === 'subscriptions.membership.cancelled' || topic === 'subscriptions.membership.expired') {
      roleAction = 'remove';
    } else {
      return;
    }
