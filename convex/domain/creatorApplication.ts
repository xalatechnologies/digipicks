...
export const list = query({
  ...
  handler: async (ctx, { tenantId, status, limit }) => {
    const applications = await ctx.runQuery(components.creatorApplication.functions.list, {
...
export const get = query({
  ...
  handler: async (ctx, { id }) => {
    const app = await ctx.runQuery(components.creatorApplication.functions.get, { id });
    if (!app) return null;

    // Enrich with user data
    const user = app.userId ? await ctx.db.get(app.userId as Id<'users'>) : null;
...
export const getMyApplication = query({
  ...
  handler: async (ctx, { tenantId, userId }) => {
    return ctx.runQuery(components.creatorApplication.functions.getByUser, {
      tenantId: tenantId as string,
...
export const submit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.runMutation(components.creatorApplication.functions.submit, {
    ...
    // Event bus
    await emitEvent(ctx, 'creator-application.application.submitted', args.tenantId as string, 'creatorApplication', {
...
export const approve = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Delegate to component
    const result = await ctx.runMutation(components.creatorApplication.functions.approve, {
      id: args.id,
    ...
    const app = await ctx.runQuery(components.creatorApplication.functions.get, { id: args.id });
    ...
    // Event bus
    await emitEvent(ctx, 'creator-application.application.approved', args.tenantId as string, 'creatorApplication', {
...
export const reject = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.runMutation(components.creatorApplication.functions.reject, {
      id: args.id,
      reviewedBy: args.reviewedBy as string,
      reviewNote: args.reviewNote,
    });
    ...
    await emitEvent(ctx, 'creator-application.application.rejected', args.tenantId as string, 'creatorApplication', {
      applicationId: args.id,
      userId: app?.userId,
      reviewedBy: args.reviewedBy as string,
    });
...
export const requestMoreInfo = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.runMutation(components.creatorApplication.functions.requestMoreInfo, {
      id: args.id,
      reviewedBy: args.reviewedBy as string,
      reviewNote: args.reviewNote,
    });
...
export const resubmit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.runMutation(components.creatorApplication.functions.resubmit, {
      id: args.id,
      userId: args.userId as string,
      displayName: args.displayName,
      bio: args.bio,
    ...
    await emitEvent(ctx, 'creator-application.application.resubmitted', args.tenantId as string, 'creatorApplication', {
      applicationId: args.id,
      userId: args.userId as string,
    });
