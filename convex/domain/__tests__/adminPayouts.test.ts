...
describe("adminPayouts — requestCreatorPayout", () => {
    it("creates a pending payout with fee calculation", async () => {
        ...
        const { id, platformFee, netAmount } = await t.mutation(
            api.domain.adminPayouts.requestCreatorPayout,
            {
                tenantId,
                creatorId: userId,
                amount: 100000,
    ...
    it("creates payout with zero fee when no config exists", async () => {
        ...
        const { platformFee, netAmount } = await t.mutation(
            api.domain.adminPayouts.requestCreatorPayout,
            {
                tenantId,
                creatorId: userId,
...
describe("adminPayouts — internal payout lifecycle", () => {
    it("markPayoutProcessing updates status", async () => {
        ...
        await t.mutation(api.domain.adminPayouts.markPayoutProcessing as any, {
            payoutId: id,
        });
    ...
    it("completeCreatorPayout sets status and stripeTransferId", async () => {
        ...
        await t.mutation(api.domain.adminPayouts.completeCreatorPayout as any, {
            payoutId: id,
    ...
    it("failCreatorPayout sets status and reason", async () => {
        ...
        await t.mutation(api.domain.adminPayouts.failCreatorPayout as any, {
            payoutId: id,
