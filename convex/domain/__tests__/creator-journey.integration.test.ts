 ...
 *   Creator submits listing → superadmin sees it in queue → rejecting it →
 *   creator fixes and resubmits → superadmin approves → listing published
 *
...
describe('journey/new-user-becomes-creator', () => {
  it('complete flow: signup → tenant creation → role upgrade → admin visibility', async () => {
    ...
    // Verify: user starts as "subscriber" role
    const userBefore = await t.run(async (ctx) => ctx.db.get(userId));
    expect(userBefore?.role).toBe('subscriber');
    expect(userBefore?.tenantId).toBeUndefined();

    // ── Step 3: Check slug availability ─────────────────────────────
    const slugCheck = await t.query(api.domain.tenantOnboarding.checkSlugAvailable, { slug: 'kulturhuset-oslo' });
    expect(slugCheck.available).toBe(true);

    // ── Step 4: Create tenant ───────────────────────────────────────
    const createResult = await t.mutation(api.domain.tenantOnboarding.createTenantForOwner, {
      userId,
    ...
    const slugRechk = await t.query(api.domain.tenantOnboarding.checkSlugAvailable, { slug: 'kulturhuset-oslo' });
    expect(slugRechk.available).toBe(false);

    // ── Step 7: Owner can see their tenant ──────────────────────────
    const myTenants = await t.query(api.domain.tenantOnboarding.listMyTenants, { userId });
    expect(myTenants.length).toBe(1);
    expect(myTenants[0].name).toBe('Kulturhuset Oslo');
    expect(myTenants[0].isOwner).toBe(true); // creator owns their tenant

    // ── Step 8: Platform admin sees the new tenant ──────────────────
    const allTenants = await t.query(api.domain.tenantOnboarding.listAllTenants, {});
    const newTenant = allTenants.find((t: any) => t.slug === 'kulturhuset-oslo');
...
describe('journey/creator-listing-lifecycle', () => {
  it('auto-approve path: create → submit → auto-approve → pause → resume → renew', async () => {
    ...
    const { tenantId } = await t.mutation(api.domain.tenantOnboarding.createTenantForOwner, {
      userId: ownerId,
      name: 'Test Venue',
      slug: 'test-venue',
    });
    ...
    const submitResult = await t.mutation(api.domain.listingModeration.submitForReview, {
    ...
    const pauseResult = await t.mutation(api.domain.listingModeration.pauseListing, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
    });
    ...
    const resumeResult = await t.mutation(api.domain.listingModeration.resumeListing, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
    });
    ...
    const renewResult = await t.mutation(api.domain.listingModeration.renewListing, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
    });
    ...
    const renewResult2 = await t.mutation(api.domain.listingModeration.renewListing, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
    });
  ...
  it('manual-review path: submit → admin approve → published', async () => {
    ...
    const approveResult = await t.mutation(api.domain.listingModeration.approveListing, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
...
describe('journey/moderation-rejection-resubmission', () => {
  it('submit → reject → request changes → resubmit → approve', async () => {
    ...
    const submit1 = await t.mutation(api.domain.listingModeration.submitForReview, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
    });
    ...
    const changeResult = await t.mutation(api.domain.listingModeration.requestChanges, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
    ...
    const submit2 = await t.mutation(api.domain.listingModeration.submitForReview, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
    });
  ...
  it('submit → reject outright', async () => {
    ...
    // Reject
    const rejectResult = await t.mutation(api.domain.listingModeration.rejectListing, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
      moderatorId: adminId as string,
...
describe('journey/subscriber-reports-listing', () => {
  it('subscriber reports → admin sees reports → admin resolves', async () => {
    const t = createJourneyTest();
    ...
    const reportResult = await t.mutation(api.domain.listingReports.reportListing, {
      resourceId: resource.id,
      tenantId: tenantId! as string,
    ...
    const reports = await t.query(api.domain.listingReports.listReports, { status: 'pending' });
    expect(reports).toBeDefined();
    // Should have at least one report
    const ourReport = reports.find((r: any) => r.reason === 'misleading');
    expect(ourReport).toBeDefined();

    // ── Step 3: Admin resolves the report ───────────────────────────
    if (ourReport) {
      const resolveResult = await t.mutation(api.domain.listingReports.resolveReport, {
...
describe('journey/multi-creator-platform-growth', () => {
  it('two creators create tenants → platform admin sees correct aggregates', async () => {
    ...
    const tenant1 = await t.mutation(api.domain.tenantOnboarding.createTenantForOwner, {
      userId: owner1Id,
      name: 'Kulturhuset',
      slug: 'kulturhuset',
    });
    ...
    const tenant2 = await t.mutation(api.domain.tenantOnboarding.createTenantForOwner, {
      userId: owner2Id,
      name: 'Festlokalet',
      slug: 'festlokalet',
    });
    ...
    expect(stats.users.total).toBe(3); // superadmin + 2 creators
    expect(stats.users.owners).toBe(2);

    // ── Each creator sees only their tenant ─────────────────────────
    const owner1Tenants = await t.query(api.domain.tenantOnboarding.listMyTenants, { userId: owner1Id });
    expect(owner1Tenants.length).toBe(1);
    expect(owner1Tenants[0].name).toBe('Kulturhuset');

    const owner2Tenants = await t.query(api.domain.tenantOnboarding.listMyTenants, { userId: owner2Id });
    ...
    const allUsers = await t.query(api.domain.platformAdmin.listAllUsers, {});
    ...
    const owner2Listings = await t.query(api.domain.listingModeration.listByTenantAndStatus, {
...
describe('journey/onboarding-progress', () => {
  it('tracks tenant onboarding steps', async () => {
    ...
    // ── Verify final state ──────────────────────────────────────────
    const updated = await t.query(api.domain.tenantOnboarding.listAllTenants, {});
    const completedTenant = updated.find((t: any) => t.slug === 'fresh-venue');
...
describe('journey/creator-payout-flow', () => {
  it('add bank account → request payout → verify balance', async () => {
    ...
    const initialAccounts = await t.query(api.domain.payouts.listBankAccounts, { tenantId: tenantId! });
    expect(initialAccounts.length).toBe(0);

    // ── Step 2: Add a bank account ──────────────────────────────────
    const addResult = await t.mutation(api.domain.payouts.addBankAccount, {
      tenantId: tenantId!,
    ...
    const accounts = await t.query(api.domain.payouts.listBankAccounts, { tenantId: tenantId! });
    expect(accounts.length).toBe(1);
    expect(accounts[0].accountNumber).toBe('1234.56.78901');
    expect(accounts[0].isDefault).toBe(true);

    // ── Step 4: Check initial balance ───────────────────────────────
    const balance = await t.query(api.domain.payouts.getBalance, { tenantId: tenantId! });
    ...
    const payoutResult = await t.mutation(api.domain.payouts.requestPayout, {
      tenantId: tenantId!,
      amount: 5000,
    ...
    const payouts = await t.query(api.domain.payouts.list, { tenantId: tenantId!, limit: 10 });
    expect(payouts.length).toBe(1);
    expect(payouts[0].amount).toBe(5000);
    expect(payouts[0].status).toBe('pending');

    // ── Step 7: Check balance reflects pending payout ───────────────
    const updatedBalance = await t.query(api.domain.payouts.getBalance, { tenantId: tenantId! });
    ...
    const allAccounts = await t.query(api.domain.payouts.listBankAccounts, { tenantId: tenantId! });
    expect(allAccounts.length).toBe(2);

    // ── Step 9: Remove second bank account ──────────────────────────
    const secondAccount = allAccounts.find((a: any) => a.accountNumber === '9876.54.32100');
    if (secondAccount) {
      const removeResult = await t.mutation(api.domain.payouts.removeBankAccount, { id: secondAccount._id });
      expect(removeResult.success).toBe(true);
    }

    const finalAccounts = await t.query(api.domain.payouts.listBankAccounts, { tenantId: tenantId! });
...
describe('journey/license-lifecycle', () => {
  it('activate trial → check status → record billing', async () => {
    ...
    // ── Setup: Create owner with tenant ─────────────────────────────
    const ownerId = await createUser(t, {
      email: 'license-creator@venue.no',
    ...
    const initialStatus = await t.query(api.domain.licensing.getStatus, { tenantId: tenantId! });
    ...
    const trialResult = await t.mutation(api.domain.licensing.activateTrial, { tenantId: tenantId! });
    expect(trialResult.success).toBe(true);

    // ── Step 3: Verify trial is active ──────────────────────────────
    const trialStatus = await t.query(api.domain.licensing.getStatus, { tenantId: tenantId! });
    ...
    const initialBilling = await t.query(api.domain.licensing.listBillingHistory, { tenantId: tenantId!, limit: 10 });
    expect(initialBilling.length).toBe(0);

    // ── Step 5: Record a billing entry ──────────────────────────────
    const billingResult = await t.mutation(api.domain.licensing.recordBilling, {
      tenantId: tenantId!,
      objectCount: 3,
      amount: 300,
      period: '2026-03',
    ...
    const billingHistory = await t.query(api.domain.licensing.listBillingHistory, { tenantId: tenantId!, limit: 10 });
    ...
    const updateResult = await t.mutation(api.domain.licensing.updateBillingStatus, {
    ...
    const updatedBilling = await t.query(api.domain.licensing.listBillingHistory, { tenantId: tenantId!, limit: 10 });
  ...
  it('cannot activate trial twice', async () => {
    ...
    const first = await t.mutation(api.domain.licensing.activateTrial, { tenantId: tenantId! });
    expect(first.success).toBe(true);

    // Second trial activation should fail
    const second = await t.mutation(api.domain.licensing.activateTrial, { tenantId: tenantId! });
