...
async function seedMembership(
    ...
    if (overrides.lastPaymentDate !== undefined || overrides.failedPaymentCount !== undefined) {
        await (t as any).mutation(
            components.subscriptions.functions.updateMembership,
            {
                id: membership.id,
    ...
    if (overrides.status && overrides.status !== "active") {
        await (t as any).mutation(
            ...
            { id: membership.id, status: overrides.status }
        );
    }
...
describe("domain/picks — create", () => {
    ...
    it("rejects inactive user", async () => {
        ...
        await expect(
            t.mutation(api.domain.picks.create, pickArgs(tenantId, inactiveId))
    ...
    it("creates audit entry on pick creation", async () => {
        ...
        const pickAudit = auditEntries.find(
            (e: any) => e.entityType === "pick" && e.entityId === result.id
        );
...
describe("domain/picks — grade", () => {
    it("grades a pick and creates audit entry", async () => {
        ...
        const gradeAudit = auditEntries.find(
