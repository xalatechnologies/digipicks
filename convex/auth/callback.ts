  ...
  handler: async (
    ...
    switch (provider) {
      ...
      case 'idporten': {
        ...
        userInfo = await fetchBankidUserInfo(tokenResponse.access_token as string);
      ...
      case 'vipps': {
        ...
        userInfo = await fetchVippsUserInfo(tokenResponse.access_token as string);
        break;
      }
      ...
      case 'google': {
        ...
        userInfo = await fetchGoogleUserInfo(tokenResponse.access_token as string);
        break;
      }
      ...
      case 'microsoft': {
        ...
        userInfo = await fetchAzureUserInfo(tokenResponse.access_token as string);
        break;
      }
...
async function ensureBackofficeAccess(ctx: any, userId: string, tenantId: string) {
  const membership = await ctx.db
    .query('tenantUsers')
    .withIndex('by_tenant_user', (q: any) => q.eq('tenantId', tenantId as any).eq('userId', userId as any))
    .first();
...
async function exchangeBankidCode(
  ...
  const response = await fetch(
    process.env.BANKID_TOKEN_URL || 'https://digilist.sandbox.signicat.com/auth/open/connect/token',
    {
      method: 'POST',
      headers: {
...
async function fetchBankidUserInfo(accessToken: string): Promise<Record<string, unknown>> {
  const response = await fetch(
    process.env.BANKID_USERINFO_URL || 'https://digilist.sandbox.signicat.com/auth/open/connect/userinfo',
...
async function exchangeVippsCode(code: string, redirectUri: string): Promise<Record<string, unknown>> {
  const clientId = process.env.VIPPS_CLIENT_ID!;
  const clientSecret = process.env.VIPPS_CLIENT_SECRET!;
  const basicAuth = btoa(`${clientId}:${clientSecret}`);

  const response = await fetch(
    process.env.VIPPS_TOKEN_URL || 'https://apitest.vipps.no/access-management-1.0/access/oauth2/token',
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
...
async function fetchVippsUserInfo(accessToken: string): Promise<Record<string, unknown>> {
  const response = await fetch(
    process.env.VIPPS_USERINFO_URL || 'https://apitest.vipps.no/vipps-userinfo-api/userinfo',
    { headers: { Authorization: `Bearer ${accessToken}` } },
  );
  return response.json();
}
...
async function exchangeAzureCode(code: string, redirectUri: string): Promise<Record<string, unknown>> {
...
async function fetchAzureUserInfo(accessToken: string): Promise<Record<string, unknown>> {
