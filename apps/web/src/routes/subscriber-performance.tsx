...
import { useSubscriberDashboard } from '@digilist-saas/sdk';
import type { SubscriberDashboard, CreatorInsight } from '@digilist-saas/sdk';
...
export function SubscriberPerformancePage() {
  ...
  const dashboard = useSubscriberDashboard(
    tenantId as any,
    userId,
    bankroll,
  );
      ...
      {!isLoading && displayStats && displayStats.totalTailed === 0 && (
        <div className={s.emptyState}>
          ...
          <Paragraph className={s.emptyText}>
            {t('performance.emptyDesc', 'Subscribe to creators and tail their picks to start tracking your performance.')}
          </Paragraph>
      ...
      {displayStats && displayStats.totalTailed > 0 && (
        <>
          <div className={s.statsGrid}>
            <StatCard
              label={t('performance.totalPicks', 'Total Picks')}
              value={String(displayStats.totalTailed)}
            />
            <StatCard
            ...
            />
            <StatCard
              label={t('performance.pending', 'Pending')}
              value={String(displayStats.pending)}
            />
          </div>
                ...
                {filteredSportBreakdown.map((sport) => (
                  <Card key={sport.sport} className={s.breakdownCard}>
                    ...
                    <div className={s.breakdownStats}>
                      <span>{sport.picks} {t('performance.picks', 'picks')}</span>
            ...
            <div className={s.section}>
              ...
              <div className={s.insightsGrid}>
                ...
                <Card className={s.insightCard}>
                  ...
                  <div className={s.insightValue}>
                    {dashboard.bankrollInsights.riskMetrics.sharpeRatio.toFixed(2)}
                  </div>
                </Card>
                {dashboard.bankrollInsights.projection.next50PicksExpected !== undefined && (
                  <Card className={s.insightCard}>
                    ...
                    <div className={`${s.insightValue} ${dashboard.bankrollInsights.projection.next50PicksExpected >= 0 ? s.statPositive : s.statNegative}`}>
