...
import { Card, Heading, Paragraph, Button, Spinner, StatusTag } from '@digipicks/ds';
...
export function PickDetailPage() {
  ...
  const { profile: creatorProfile } = useCreatorProfile(tenantId as any, creatorId);
    ...
    <div className={s.pageContainer}>
      ...
      {isLocked ? (
        <div className={s.lockedOverlay}>
          ...
          <div className={s.lockedCta}>
            ...
            <span className={s.lockTitle}>{t('picks.detail.premiumPick', 'Premium Pick')}</span>
            <span className={s.lockSubtitle}>
              {t(
                'picks.detail.subscribeToUnlock',
                'Subscribe to this creator to unlock full pick details and analysis.',
              )}
            </span>
            <Button variant="primary" data-size="md" onClick={handleSubscribe} disabled={subscribing}>
              {subscribing ? t('common.loading', 'Loading...') : t('picks.detail.subscribeBtn', 'Subscribe to Unlock')}
            </Button>
          </div>
        </div>
      ) : (
          ...
          <div className={s.detailsGrid}>
            ...
            <div className={s.detailCell}>
              ...
              <span className={s.detailValue}>
                {pick.oddsAmerican || '\u2014'} {pick.oddsDecimal != null && `(${pick.oddsDecimal.toFixed(2)})`}
              </span>
            </div>
            <div className={s.detailCell}>
              <span className={s.detailLabel}>{t('picks.detail.units', 'Units')}</span>
        ...
        <Link to={`/creator/${creatorId}`} style={{ textDecoration: 'none' }}>
          <div className={s.creatorCardInner}>
            {creatorProfile?.avatarUrl ? (
              <img src={creatorProfile.avatarUrl} alt={creatorName} className={s.creatorAvatarImg} />
            ) : (
            ...
            <div className={s.creatorMeta}>
              ...
              {stats && (
                <div className={s.creatorRecord}>
                  ...
                  {stats.winRate > 0 &&
                    ` \u00b7 ${(stats.winRate * 100).toFixed(0)}% ${t('picks.detail.winRate', 'win rate')}`}
      ...
      {!isSubscribed && creatorId && (
        <Card className={s.creatorCard} style={{ textAlign: 'center' }}>
          ...
          <Button variant="primary" data-size="md" onClick={handleSubscribe} disabled={subscribing}>
            {subscribing ? t('common.loading', 'Loading...') : t('picks.detail.subscribeCta', 'Subscribe Now')}
          </Button>
        </Card>
      )}
    </div>
