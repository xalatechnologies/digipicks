...
import { useMySubscription } from '@digipicks/sdk';
  ...
  return (
    <div className={s.pageContainer}>
        ...
        <Card className={s.detailsCard}>
          ...
          <div className={s.detailRow}>
            ...
            <span className={s.detailValue} style={{ color: 'var(--ds-color-success-text-default)' }}>
              {subscription.isTrialing ? t('checkout.trialing', 'Free Trial') : t('checkout.active', 'Active')}
            </span>
          </div>
      ...
      {waitingTooLong && !subscription && (
        <Card className={s.detailsCard}>
          <Paragraph data-size="sm" style={{ color: 'var(--ds-color-warning-text-default)' }}>
            {t(
              'checkout.activationDelay',
              'Your payment was successful but activation is taking a moment. Your subscription will appear shortly — you can safely navigate away.',
            )}
          </Paragraph>
        </Card>
      )}
      ...
      <div className={s.actions}>
        ...
        <Button variant="secondary" data-size="md" onClick={() => navigate('/picks')}>
