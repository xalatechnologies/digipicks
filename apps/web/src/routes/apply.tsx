...
import { useMyCreatorApplication, useSubmitCreatorApplication, useResubmitCreatorApplication } from '@digipicks/sdk';
...
export function ApplyPage() {
      ...
      <div className={styles.applyPage}>
        <div className={styles.loginPrompt}>
          ...
          <Stack
            ...
            style={{ justifyContent: 'center', marginTop: 'var(--ds-size-4)' }}
          >
            <Button onClick={() => navigate('/login')}>{t('common.login', 'Sign in')}</Button>
    ...
    return (
      <div className={styles.applyPage}>
        <Card className={styles.statusCard}>
          ...
          <Paragraph data-size="md" style={{ marginTop: 'var(--ds-size-3)' }}>
            {t(
              'apply.submittedDescription',
              "We'll review your application and get back to you soon. Check back here for updates.",
            )}
          </Paragraph>
          <Button variant="secondary" onClick={() => navigate('/')} style={{ marginTop: 'var(--ds-size-4)' }}>
    ...
    return (
      <div className={styles.applyPage}>
        <Card className={styles.statusCard}>
          ...
          <Paragraph
            data-size="sm"
            style={{ marginTop: 'var(--ds-size-3)', color: 'var(--ds-color-neutral-text-subtle)' }}
          >
          ...
          {application.status === 'approved' && (
            <Paragraph data-size="md" style={{ marginTop: 'var(--ds-size-3)' }}>
              {t(
                'apply.approvedMessage',
                "Congratulations! You're now a verified creator. Head to your dashboard to start posting picks.",
              )}
            </Paragraph>
          )}
        </Card>
  ...
  const isResubmission =
    application && (application.status === 'more_info_requested' || application.status === 'rejected');
    ...
    <div className={styles.applyPage}>
      ...
      <Paragraph data-size="md" style={{ marginTop: 'var(--ds-size-2)' }}>
        {isResubmission
          ...
          : t(
              'apply.description',
              "Share your expertise with sports bettors. Fill out the form below and we'll review your application.",
            )}
      </Paragraph>
        ...
        <form onSubmit={isResubmission ? handleResubmit : handleSubmit}>
          <Stack direction="vertical" spacing="var(--ds-size-4)">
            ...
            <Textarea
              ...
              placeholder={t(
                'apply.performanceProofPlaceholder',
                'Describe your betting track record, win rates, ROI, etc.',
              )}
              rows={3}
            />
            ...
            <div className={styles.submitRow}>
              ...
              <Button variant="primary" type="submit" loading={isSubmitting} disabled={isSubmitting}>
                {isResubmission ? t('apply.resubmit', 'Resubmit application') : t('apply.submit', 'Submit application')}
              </Button>
            </div>
          </Stack>
        </form>
