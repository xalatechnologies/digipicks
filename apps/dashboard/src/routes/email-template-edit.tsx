...
import { useEmailTemplates, useCreateEmailTemplate, useUpdateEmailTemplate, useSendTestEmail } from '@digipicks/sdk';
...
export function EmailTemplateEditPage() {
  ...
  const categoryMeta = useMemo(
    (): Record<string, { label: string; statusColor: BadgeColor; description: string }> => ({
      booking: {
        label: t('emailTemplates.categoryBooking'),
        statusColor: 'info',
        description: t('emailTemplates.categoryBookingDesc'),
      },
      notification: {
        label: t('emailTemplates.categoryNotification'),
        statusColor: 'warning',
        description: t('emailTemplates.categoryNotificationDesc'),
      },
      reminder: {
        label: t('emailTemplates.categoryReminder'),
        statusColor: 'neutral',
        description: t('emailTemplates.categoryReminderDesc'),
      },
      invoice: {
        label: t('emailTemplates.categoryInvoice'),
        statusColor: 'success',
        description: t('emailTemplates.categoryInvoiceDesc'),
      },
      welcome: {
        label: t('emailTemplates.categoryWelcome'),
        statusColor: 'info',
        description: t('emailTemplates.categoryWelcomeDesc'),
      },
      system: {
        label: t('emailTemplates.categorySystem'),
        statusColor: 'neutral',
        description: t('emailTemplates.categorySystemDesc'),
      },
    }),
    [t],
  );
  ...
  const previewHtml = useMemo(() => {
    ...
    templateVariables.forEach((group) =>
      group.tokens.forEach((v) => {
        sampleData[v.token] = v.example;
      }),
    );
    let output = formBody;
                  ...
                  <Stack direction="horizontal" spacing="var(--ds-size-6)">
                    ...
                    {existingTemplate.isDefault && (
                      <MetadataRow
                        ...
                        badge={
                          <StatusTag color="info" size="sm">
                            {t('emailTemplates.standard')}
                          </StatusTag>
                        }
                        size="sm"
                      />
          ...
          <Stack direction="vertical" spacing="var(--ds-size-5)">
            <Stack direction="vertical" spacing="var(--ds-size-1)">
              <Heading level={2} data-size="sm">
                {t('emailTemplates.sectionContent')}
              </Heading>
                  ...
                  {templateVariables.map((group) => (
                    <Stack key={group.group} direction="vertical" spacing="var(--ds-size-1)">
                      ...
                      {group.tokens.map((v) => (
                        ...
                        >
                          <code className={styles.variableTokenCode}>{v.token}</code>
                        </Button>
                      ))}
                    </Stack>
                  ))}
          ...
          <Stack direction="vertical" spacing="var(--ds-size-5)">
            <Stack direction="vertical" spacing="var(--ds-size-1)">
              <Heading level={2} data-size="2xs">
                {t('emailTemplates.sectionReview')}
              </Heading>
              <Paragraph data-size="xs" className={styles.subtleText}>
              ...
              <Stack direction="vertical" spacing="var(--ds-size-3)" className={styles.previewBody}>
                ...
                <Card className={styles.previewEmailHeader}>
                  <Stack direction="vertical" spacing="var(--ds-size-1)">
                    <MetadataRow
                      icon={<MailIcon />}
                      label={t('emailTemplates.previewFrom')}
                      value={import.meta.env.VITE_EMAIL_FROM || 'noreply@example.com'}
                      size="sm"
                    />
                    <MetadataRow
                      icon={<MailIcon />}
                      label={t('emailTemplates.previewTo')}
                      value={user?.email || 'bruker@example.no'}
                      size="sm"
                    />
                    <MetadataRow
                      icon={<MailIcon />}
                      label={t('emailTemplates.previewSubject')}
                      value={formSubject || t('emailTemplates.previewNoSubject')}
                      size="sm"
                    />
                  </Stack>
                </Card>
            ...
            {user?.email && (
              <Stack direction="horizontal" spacing="var(--ds-size-3)" align="center">
                ...
                >
                  <SendIcon />
                  {isSendingTest
                    ? t('emailTemplates.buttonSending')
                    : t('emailTemplates.buttonSendTestTo', { email: user.email })}
                </Button>
              </Stack>
            )}
          </Stack>
        ...
        <div className={styles.header}>
          <div className={styles.headerLeft}>
            ...
            <Paragraph data-size="sm" className={styles.wizardSubtitle}>
              {isEditMode ? t('emailTemplates.wizardEditDesc') : t('emailTemplates.wizardCreateDesc')}
            </Paragraph>
          </div>
        </div>
        ...
        <div className={styles.wizardFooter}>
          ...
          <div className={styles.footerCenter}>
            <Button
              variant="secondary"
              onClick={handleSave}
              disabled={isSaving || !formName.trim() || !formSubject.trim()}
            >
