...
import type { ModerationQueuePick, ModerationStatus, PickReport } from '@digilist-saas/sdk';
...
export default function PickModerationPage() {
  ...
  // Handlers
  const handleModerate = useCallback(async (status: ModerationStatus) => {
  ...
  const columns: DataTableColumn<ModerationQueuePick>[] = useMemo(() => [
    ...
    },
    {
      ...
      header: 'Status',
      render: (row) => (
        <Badge data-color={getStatusColor(row.moderationStatus)}>
  ...
  return (
    <div className={styles.page}>
      <DashboardPageHeader
        title="Pick Moderation"
        description="Review flagged and reported picks"
      />
      ...
      {selectedPick && (
        <Drawer
          open={!!selectedPick}
          onClose={() => { setSelectedPick(null); setModerationNote(''); }}
          title={`Review: ${selectedPick.event}`}
        >
          <div className={styles.drawerContent}>
            <DrawerSection title="Pick Details">
              <Stack>
                <Paragraph><strong>Creator:</strong> {selectedPick.creator?.displayName ?? selectedPick.creatorId}</Paragraph>
                <Paragraph><strong>Sport:</strong> {selectedPick.sport}</Paragraph>
                <Paragraph><strong>Selection:</strong> {selectedPick.selection}</Paragraph>
                <Paragraph><strong>Odds:</strong> {selectedPick.oddsAmerican} ({selectedPick.oddsDecimal})</Paragraph>
                <Paragraph><strong>Units:</strong> {selectedPick.units}</Paragraph>
                <Paragraph><strong>Confidence:</strong> {selectedPick.confidence}</Paragraph>
                <Paragraph><strong>Result:</strong> {selectedPick.result}</Paragraph>
                <Paragraph><strong>Reports:</strong> {selectedPick.reportCount ?? 0}</Paragraph>
                <Paragraph><strong>Current Status:</strong> <Badge data-color={getStatusColor(selectedPick.moderationStatus)}>{selectedPick.moderationStatus ?? 'clean'}</Badge></Paragraph>
                {selectedPick.moderationNote && (
                  <Paragraph><strong>Previous Note:</strong> {selectedPick.moderationNote}</Paragraph>
                )}
              </Stack>
            </DrawerSection>
                ...
                reports.map((report: PickReport) => (
                  <div key={report._id} className={styles.reportItem}>
                    <div className={styles.reportHeader}>
                      ...
                      <Badge data-color={report.status === 'pending' ? 'warning' : 'neutral'}>
                        {report.status}
                      </Badge>
                    </div>
                    <Paragraph>
            ...
            <DrawerSection title="Moderation Action">
              <Stack>
                ...
                <div className={styles.moderationActions}>
                  ...
                  <Button
                    variant="secondary"
                    onClick={() => handleModerate('under_review')}
                  >
                    Mark Under Review
                  </Button>
                  <Button
                    variant="tertiary"
                    onClick={() => handleModerate('clean')}
                  >
                    Clear (Clean)
                  </Button>
                </div>
