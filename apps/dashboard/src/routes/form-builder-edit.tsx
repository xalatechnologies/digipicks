...
import { useFormDefinitions, useCreateFormDefinition, useUpdateFormDefinition } from '@digilist-saas/sdk';
...
export function FormBuilderEditPage() {
    ...
    const categoryMeta: Record<string, { label: string; statusColor: BadgeColor; description: string }> = useMemo(() => ({
        booking: { label: t('formBuilder.categoryBooking'), statusColor: 'info', description: t('formBuilder.sectionBasicInfoDesc') },
        application: { label: t('formBuilder.categoryApplication'), statusColor: 'warning', description: t('formBuilder.sectionBasicInfoDesc') },
        feedback: { label: t('formBuilder.categoryFeedback'), statusColor: 'success', description: t('formBuilder.sectionBasicInfoDesc') },
        registration: { label: t('formBuilder.categoryRegistration'), statusColor: 'neutral', description: t('formBuilder.sectionBasicInfoDesc') },
        survey: { label: t('formBuilder.categorySurvey'), statusColor: 'info', description: t('formBuilder.sectionBasicInfoDesc') },
    }), [t]);
    ...
    const addField = useCallback((type: string) => {
        ...
        const newField: FormField = {
            ...
            required: false,
            options: ['select', 'radio', 'checkbox'].includes(type) ? ['Alternativ 1', 'Alternativ 2', 'Alternativ 3'] : undefined,
        };
    ...
    }, [formName, formDescription, formCategory, fields, formSuccessMessage, existingForm, isEditMode, user, navigate, createFormDefinition, updateFormDefinition]);
        ...
        switch (currentStepId) {
                                ...
                                <div className={styles.cardInner}>
                                    ...
                                    <Stack direction="horizontal" spacing="var(--ds-size-6)">
                                        <MetadataRow
                                            ...
                                            value={existingForm.lastModified
                                        ...
                                        <MetadataRow
                                            ...
                                            badge={<StatusTag color={existingForm.isPublished ? 'success' : 'warning'} size="sm">{existingForm.isPublished ? t('formBuilder.published') : t('formBuilder.draft')}</StatusTag>}
                                            size="sm"
                                        />
                                    </Stack>
                                </div>
                ...
                return (
                    <Stack direction="vertical" spacing="var(--ds-size-5)">
                        <Stack direction="vertical" spacing="var(--ds-size-1)">
                            <Heading level={2} data-size="sm">{t('formBuilder.fieldsTitle')}</Heading>
                        ...
                        <Stack direction="horizontal" spacing="var(--ds-size-4)">
                            ...
                            <Stack direction="vertical" spacing="var(--ds-size-2)" className={styles.canvasArea}>
                                <Stack direction="horizontal" justify="between" align="center">
                                    ...
                                    <Tag data-size="sm" data-color="neutral">{fields.length} {t('formBuilder.fieldsTitle')}</Tag>
                                </Stack>
                                        ...
                                        return (
                                            <Card
                                                ...
                                                style={{
                                                    ...
                                                    boxShadow: isEditing ? '0 0 0 3px color-mix(in srgb, var(--ds-color-brand-1-base-default) 15%, transparent)' : undefined,
                                                }}
                                                ...
                                                <Stack direction="vertical" spacing="var(--ds-size-2)">
                                                    ...
                                                    <Stack direction="horizontal" justify="between" align="center">
                                                        <Stack direction="horizontal" spacing="var(--ds-size-2)" align="center">
                                                            <MoreVerticalIcon style={{ cursor: 'grab', color: 'var(--ds-color-neutral-text-subtle)', width: '16px', height: '16px' }} />
                                                            <StatusTag color="neutral" size="sm">{fieldType?.label || field.type}</StatusTag>
                                                            <Paragraph data-size="sm" className={styles.fontBold}>{field.label}</Paragraph>
                                                            {field.required && (
                                                                <StatusTag color="danger" size="sm">{t('formBuilder.required')}</StatusTag>
                                                            )}
                                                        </Stack>
                                                        <Stack direction="horizontal" spacing="var(--ds-size-1)">
                                                            {idx > 0 && (
                                                                <Button variant="tertiary" data-size="sm" onClick={(e: React.MouseEvent) => { e.stopPropagation(); moveField(idx, 'up'); }} aria-label={t('formBuilder.moveUp')}>↑</Button>
                                                            )}
                                                            {idx < fields.length - 1 && (
                                                                <Button variant="tertiary" data-size="sm" onClick={(e: React.MouseEvent) => { e.stopPropagation(); moveField(idx, 'down'); }} aria-label={t('formBuilder.moveDown')}>↓</Button>
                                                            )}
                                                            <Button variant="tertiary" data-size="sm" data-color="danger" onClick={(e: React.MouseEvent) => { e.stopPropagation(); removeField(field.id); }} aria-label={t('formBuilder.removeField')}>
                                                                <TrashIcon />
                                                            </Button>
                                                        </Stack>
                                                    </Stack>
                                                            ...
                                                            <Stack direction="vertical" spacing="var(--ds-size-3)">
                                                                <Grid columns="1fr 1fr" gap="var(--ds-size-3)">
                                                                    ...
                                                                    <Textfield
                                                                        ...
                                                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateField(field.id, { placeholder: e.target.value })}
                                                                        onClick={(e: React.MouseEvent) => e.stopPropagation()}
                                                                    />
                                                                </Grid>
                                                                {field.options && (
                                                                    <Textfield
                                                                        ...
                                                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateField(field.id, { options: e.target.value.split(',').map((s: string) => s.trim()).filter(Boolean) })}
                                                                        onClick={(e: React.MouseEvent) => e.stopPropagation()}
                                                                ...
                                                                <Textfield
                                                                    ...
                                                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateField(field.id, { helpText: e.target.value })}
                                                                    onClick={(e: React.MouseEvent) => e.stopPropagation()}
                                                                ...
                                                                <Stack direction="horizontal" spacing="var(--ds-size-2)" align="center">
                                                                    <Button
                                                                        ...
                                                                        data-size="sm"
                                                                        onClick={(e: React.MouseEvent) => { e.stopPropagation(); updateField(field.id, { required: !field.required }); }}
                                                                    >
                                                                        {field.required ? `✓ ${t('formBuilder.requiredToggle')}` : t('formBuilder.optionalToggle')}
                                                                    </Button>
                                                                </Stack>
                                                            </Stack>
                                                        </Card>
            ...
            case 'preview':
                return (
                    <Stack direction="vertical" spacing="var(--ds-size-5)">
                        <Stack direction="vertical" spacing="var(--ds-size-1)">
                            <Heading level={2} data-size="sm">{t('formBuilder.previewTitle')}</Heading>
                            <Paragraph data-size="sm" className={styles.subtleText}>
                        ...
                        {fields.length === 0 ? (
                            ...
                            <Card className={styles.previewCard}>
                                <Stack direction="vertical" spacing="var(--ds-size-4)">
                                    <Stack direction="vertical" spacing="var(--ds-size-1)">
                                        <Heading data-size="sm" className={styles.noMargin}>{formName || t('formBuilder.untitled')}</Heading>
                                        {formDescription && (
                                            <Paragraph data-size="sm" className={styles.subtleText}>{formDescription}</Paragraph>
                                        )}
                                    </Stack>
                                    {fields.map((field) => {
                                        if (field.type === 'divider') return <div key={field.id} className={styles.dividerPreview} />;
                                        if (field.type === 'heading') return <Heading key={field.id} data-size="2xs" className={styles.noMargin}>{field.label}</Heading>;
                                        if (field.type === 'paragraph') return <Paragraph key={field.id} data-size="sm" className={styles.subtleText}>{field.label}</Paragraph>;
                                    ...
                                    {formSuccessMessage && (
                                        <Card className={styles.successPreview}>
                                            <Paragraph data-size="sm" className={styles.noMargin}>{formSuccessMessage}</Paragraph>
                                        </Card>
                                    )}
                                </Stack>
                            </Card>
            ...
            case 'review':
                return (
                    <Stack direction="vertical" spacing="var(--ds-size-5)">
                        <Stack direction="vertical" spacing="var(--ds-size-1)">
                            <Heading level={2} data-size="2xs">{t('formBuilder.reviewTitle')}</Heading>
                        ...
                        <Card className={styles.reviewCard}>
                            <Stack direction="vertical" spacing="var(--ds-size-3)">
                                ...
                                />
                                <MetadataRow
                                    label={t('formBuilder.reviewFieldCount')}
                        ...
                        {fields.length > 0 && (
                            <Card className={styles.reviewFieldsCard}>
                                <Stack direction="vertical" spacing="var(--ds-size-2)">
                                    <Heading data-size="2xs" className={styles.noMargin}>{t('formBuilder.fieldsTitle')} ({fields.length})</Heading>
                                    {fields.map((field) => {
                                        const fieldType = FIELD_TYPES.find((ft) => ft.id === field.type);
                                        return (
                                            <Stack key={field.id} direction="horizontal" spacing="var(--ds-size-2)" align="center">
                                                <StatusTag color="neutral" size="sm">{fieldType?.label || field.type}</StatusTag>
                                                <Paragraph data-size="sm" className={styles.noMargin}>{field.label}</Paragraph>
                                                {field.required && <StatusTag color="danger" size="sm">{t('formBuilder.required')}</StatusTag>}
                                            </Stack>
                                        );
                                    })}
                                </Stack>
                ...
                <div className={styles.header}>
                    <div className={styles.headerLeft}>
                        ...
                        <Paragraph data-size="sm" className={styles.wizardSubtitle}>
                            {isEditMode
                                ? t('formBuilder.editSubtitle')
                                : t('formBuilder.createSubtitle')}
                        </Paragraph>
                    </div>
                </div>
