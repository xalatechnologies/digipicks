...
import type { WebhookSubscription } from '@digilist-saas/sdk';
...
export function WebhookSettingsPage() {
    ...
    const handleDelete = useCallback(async (subId: string) => {
    ...
    const handleToggle = useCallback(async (subId: string, isActive: boolean) => {
        if (!tenantId || !user?.id) return;
        try {
            await toggleSub({
                tenantId: tenantId as any,
    ...
    const columns: DataTableColumn<WebhookSubscription>[] = useMemo(() => [
        {
            ...
            cell: (row: WebhookSubscription) => (
                <Paragraph data-size="xs" style={{ maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {row.url}
                </Paragraph>
            ),
        },
        ...
        },
        {
            ...
            cell: (row: WebhookSubscription) =>
                row.lastDeliveredAt ? formatDateTime(row.lastDeliveredAt) : '—',
        },
        {
            id: 'actions',
            header: '',
                ...
                {showCreate && (
                    <Card>
                        <Stack direction="vertical" spacing="var(--ds-size-4)">
                            <Heading level={2} data-size="xs">Ny webhook</Heading>
