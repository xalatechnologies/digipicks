...
import type { Id, CreatorApplication, ApplicationStatus } from '@digipicks/sdk';
...
function getStatusLabel(status: ApplicationStatus, t: (key: string, fallback: string) => string): string {
  switch (status) {
    case 'pending':
      return t('creatorApps.status.pending', 'Pending');
    case 'approved':
      return t('creatorApps.status.approved', 'Approved');
    case 'rejected':
      return t('creatorApps.status.rejected', 'Rejected');
    case 'more_info_requested':
      return t('creatorApps.status.moreInfo', 'More info requested');
    default:
      return status;
  }
}
  ...
  return (
    <Stack direction="vertical" spacing="var(--ds-size-4)">
      <Heading level={3} data-size="sm">
        ...
        <VerificationBadge verified={application.status === 'approved'} verifiedAt={application.reviewedAt} size="md" />
      </Heading>
      ...
      <div className={styles.detailGrid}>
        ...
        <Paragraph data-size="sm">{new Date(application.submittedAt).toLocaleDateString()}</Paragraph>
        ...
        {application.resubmittedAt && (
          <>
            ...
            <Paragraph data-size="sm">{new Date(application.resubmittedAt).toLocaleDateString()}</Paragraph>
          </>
        )}
      </div>
...
export function CreatorApplicationsPage() {
  ...
  // Data
  const { applications, isLoading } = useCreatorApplications(tenantId as Id<'tenants'> | undefined);
          ...
          {filtered.map((app) => (
            <CrudListItem
              ...
              tags={[
                ...
                ...(app.status === 'approved'
                  ? [{ label: t('creatorApps.verified', 'Verified'), color: 'success' }]
                  : []),
              ]}
      ...
      {detailId && (
        <div className={styles.dialogOverlay} onClick={() => setDetailId(null)}>
          <Card className={styles.dialogCard} onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            <ApplicationDetail applicationId={detailId} onClose={() => setDetailId(null)} t={t} />
          </Card>
        </div>
      )}
        ...
        <div className={styles.dialogOverlay} onClick={() => setActionDialog(null)}>
          <Card className={styles.dialogCard} onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            <Stack direction="vertical" spacing="var(--ds-size-4)">
              ...
              <Paragraph data-size="sm">{actionDialog.application.displayName}</Paragraph>
              <div>
