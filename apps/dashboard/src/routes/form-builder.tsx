...
import { useFormDefinitions as useFormDefinitionsQuery, useCreateFormDefinition, useDeleteFormDefinition } from '@digilist-saas/sdk';
...
export function FormBuilderPage() {
    ...
    const filteredForms = useMemo(() => {
        ...
        result = [...result].sort((a, b) => {
            switch (selectedSort) {
                ...
                case 'name': return a.name.localeCompare(b.name, 'nb');
                case 'submissions': return (b.submissionCount ?? 0) - (a.submissionCount ?? 0);
    ...
    const handleEditForm = useCallback((form: FormDefinition) => {
    ...
    const handleDuplicate = useCallback(async (form: FormDefinition) => {
    ...
    const handleViewSubmissions = useCallback((formId: string) => {
        ...
        () => [
            {
                ...
                render: (row) => (
                    <Stack direction="horizontal" spacing="var(--ds-size-3)" align="center">
                        <IconBox
                            icon={<EditIcon />}
                            variant={row.isPublished ? 'accent' : 'neutral'}
                            size="sm"
                        />
                        <Stack direction="vertical" spacing="0">
                            <Paragraph data-size="sm" className={styles.cellTitle}>{row.name}</Paragraph>
                            <Paragraph data-size="xs" className={styles.cellSubtle}>
                                {row.description?.slice(0, 50)}{(row.description?.length ?? 0) > 50 ? '...' : ''}
                            </Paragraph>
                        </Stack>
                    </Stack>
                ),
                ...
                render: (row) => (
                    <Dropdown.TriggerContext>
                        ...
                        <Dropdown placement="bottom-end">
                            <Dropdown.List>
                                <Dropdown.Item>
                                    <Dropdown.Button onClick={(e: React.MouseEvent) => { e.stopPropagation(); handleEditForm(row); }}>
                                ...
                                <Dropdown.Item>
                                    <Dropdown.Button onClick={(e: React.MouseEvent) => { e.stopPropagation(); handleViewSubmissions(row.id); }}>
        ...
        <PageContentLayout>
            ...
            <Grid columns="repeat(4, 1fr)" gap="var(--ds-size-4)">
                {[
                    ...
                    { icon: <CheckCircleIcon />, label: t('formBuilder.publishedForms'), value: stats.published, variant: 'success' as const },
                    { icon: <ChartIcon />, label: t('formBuilder.totalSubmissions'), value: stats.totalSubmissions, variant: 'warning' as const },
                    { icon: <CalendarIcon />, label: t('formBuilder.totalFields'), value: stats.totalFields, variant: 'neutral' as const },
                ].map((stat) => (
            ...
            {selectedIds.length > 0 && (
                <Stack direction="horizontal" align="center" spacing="var(--ds-size-3)" className={styles.bulkBar}>
                    <Paragraph data-size="sm" className={styles.bulkBarText}>{t('common.nSelected', { count: selectedIds.length })}</Paragraph>
