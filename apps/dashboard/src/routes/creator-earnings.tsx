...
import { Card, Heading, Paragraph, Spinner, PageContentLayout, EmptyState, StatusTag } from '@digipicks/ds';
import { useT } from '@digipicks/i18n';
import { useMyEarningsSummary, useMyEarningsHistory, useMyPayouts } from '@digipicks/sdk';
...
function payoutStatusLabel(status: string, t: (key: string, fallback: string) => string): string {
  switch (status) {
    case 'completed':
      return t('earnings.payout.status.completed', 'Completed');
    case 'processing':
      return t('earnings.payout.status.processing', 'Processing');
    case 'pending':
      return t('earnings.payout.status.pending', 'Pending');
    case 'failed':
      return t('earnings.payout.status.failed', 'Failed');
    default:
      return status;
  }
}
    ...
    <PageContentLayout data-gap="sm">
      ...
      {history.length === 0 ? (
        <EmptyState
          ...
          description={t(
            'earnings.history.empty.description',
            'Revenue will appear here as subscribers join your plans.',
          )}
        />
      ) : (
        <Card>
          {history.map((period) => (
            <div key={period._id} className={styles.historyRow}>
              ...
              <span className={styles.historyAmount}>{formatCurrency(period.netEarnings, period.currency)}</span>
            </div>
          ))}
        </Card>
      )}
      ...
      {payouts.length === 0 ? (
        <EmptyState
          ...
          description={t(
            'earnings.payouts.empty.description',
            'Payouts will appear here once your earnings are transferred.',
          )}
        />
      ) : (
        <Card>
          {payouts.map((payout) => (
            <div key={payout._id} className={styles.payoutRow}>
              <div className={styles.payoutInfo}>
                <span className={styles.payoutAmount}>{formatCurrency(payout.netAmount, payout.currency)}</span>
                <span className={styles.payoutDate}>{new Date(payout.requestedAt).toLocaleDateString()}</span>
              </div>
