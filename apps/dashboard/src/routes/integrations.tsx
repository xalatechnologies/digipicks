...
import { useIntegrationConfigs, useCreateIntegrationConfig, useRemoveIntegrationConfig } from '@digipicks/sdk';
import type { IntegrationConfigRecord } from '@digipicks/sdk';
...
export function IntegrationsPage() {
  ...
  const typeMeta: Record<
    string,
    { label: string; statusColor: BadgeColor; iconVariant: 'accent' | 'success' | 'warning' | 'danger' | 'neutral' }
  > = useMemo(
    () => ({
  ...
  const integrations: IntegrationConfigRecord[] = useMemo(() => rawConfigs ?? [], [rawConfigs]);
  ...
  const filteredIntegrations = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter((i) => i.name.toLowerCase().includes(q) || i.integrationType.toLowerCase().includes(q));
    }
    return result;
  ...
  const handleDuplicate = useCallback(
    async (integration: IntegrationConfigRecord) => {
      if (!tenantId) return;
      try {
    ...
    () => [
        ...
        render: (row) => {
          ...
          return (
            <Stack direction="horizontal" spacing="var(--ds-size-3)" align="center">
              <IconBox icon={<ExternalLinkIcon />} variant={meta?.iconVariant ?? 'neutral'} size="sm" />
      ...
      },
      {
        ...
        render: (row) => {
          ...
          return (
            <StatusTag color={meta.color} size="sm">
              {meta.label}
            </StatusTag>
          );
        },
        ...
        {[
          {
            ...
            label: t('integrations.totalIntegrations'),
            value: stats.total,
            variant: 'accent' as const,
          },
          {
            icon: <CheckCircleIcon />,
            label: t('integrations.activeIntegrations'),
            value: stats.active,
      ...
      <PillTabs
        tabs={TYPE_OPTIONS.map((opt) => {
          const count =
            opt.id === 'all'
              ? integrations.length
              : integrations.filter((i: IntegrationConfigRecord) => i.integrationType === opt.id).length;
      ...
      {isLoading ? (
        ...
        <Card data-color="neutral" style={{ padding: 'var(--ds-size-12)' }}>
          <EmptyState
            ...
            description={searchQuery ? t('integrations.noResultsDesc') : t('integrations.noIntegrationsDesc')}
            action={
              !searchQuery ? (
                <Button data-size="sm" onClick={handleNewIntegration}>
                  <PlusIcon />
