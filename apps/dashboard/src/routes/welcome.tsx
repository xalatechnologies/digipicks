...
import { Card, Heading, Paragraph, Button, Stack, Tag, CheckIcon, PlusIcon, PageContentLayout } from '@digipicks/ds';
import { useAuth, useTenantContext, useSetPageTitle } from '@digipicks/app-shell';
...
export function WelcomePage() {
  ...
  const steps = [
    ...
    {
      key: 'listing',
      title: 'Opprett ditt første lokale',
      desc: 'Legg til bilder, beskrivelse og priser.',
      link: '/listings/new',
      action: 'Opprett lokale',
    },
    {
      key: 'pricing',
      title: 'Sett opp priser',
      desc: 'Konfigurer prisgrupper, rabattkoder og tillegg.',
      link: '/pricing-rules',
      action: 'Gå til priser',
    },
    {
      key: 'contact',
      title: 'Legg til kontaktinfo',
      desc: 'Telefon, e-post og åpningstider.',
      link: '/settings',
      action: 'Gå til innstillinger',
    },
  ];
  ...
  return (
    <PageContentLayout>
      <Stack
        direction="vertical"
        spacing="var(--ds-size-8)"
        style={{ paddingTop: 'var(--ds-size-6)', paddingBottom: 'var(--ds-size-10)' }}
      >
        {/* Hero */}
        <Stack
          direction="vertical"
          ...
          align="center"
          style={{ textAlign: 'center', padding: 'var(--ds-size-8) 0' }}
        >
          <Heading level={1} data-size="xl">
            Velkommen, {userName}!
          </Heading>
          <Paragraph data-size="lg" data-color="subtle">
            Du er nå klar til å komme i gang som utleier på plattformen.
          </Paragraph>
          <Tag data-size="md" data-color={planInfo.color}>
            {planInfo.label}
          </Tag>
        </Stack>
