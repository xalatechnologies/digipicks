...
type ProviderType = 'payment' | 'accounting' | 'calendar' | 'email' | 'crm' | 'webhook' | 'ticketing' | 'access' | 'erp';
...
export function IntegrationEditPage() {
    ...
    const PROVIDER_CATALOG = useMemo(() => [
        ...
        { id: 'tripletex', name: 'Tripletex', type: 'accounting', description: t('integrationEdit.providerTripletexDesc') },
        { id: 'poweroffice', name: 'PowerOffice Go', type: 'accounting', description: t('integrationEdit.providerPowerofficeDesc') },
        ...
        { id: 'ticketmaster', name: 'TicketMaster', type: 'ticketing', description: t('integrationEdit.providerTicketmasterDesc') },
        ...
        { id: 'google-calendar', name: 'Google Calendar', type: 'calendar', description: t('integrationEdit.providerGoogleCalDesc') },
        { id: 'outlook', name: 'Microsoft Outlook', type: 'calendar', description: t('integrationEdit.providerOutlookDesc') },
        ...
        { id: 'custom-webhook', name: t('integrationEdit.providerCustomWebhook'), type: 'webhook', description: t('integrationEdit.providerCustomWebhookDesc') },
        { id: 'zapier', name: 'Zapier', type: 'webhook', description: t('integrationEdit.providerZapierDesc') },
    ], [t]);

    const TYPE_META: Record<ProviderType, { label: string; iconVariant: 'success' | 'accent' | 'warning' | 'neutral' }> = useMemo(() => ({
    ...
    useEffect(() => {
        ...
        const matchedProvider = PROVIDER_CATALOG.find(
            (p) => p.id === existingConfig.integrationType
        ) ?? PROVIDER_CATALOG.find(
            (p) => p.type === existingConfig.integrationType
        );
    ...
    const provider = useMemo(() =>
        PROVIDER_CATALOG.find((p) => p.id === selectedProvider),
        [selectedProvider]
    );
    ...
    const canProceed = useMemo(() => {
        switch (currentStep) {
            case 'general': return Boolean(selectedProvider && integrationName.trim());
    ...
    }, [user, provider, isEditMode, slug, integrationName, environment, apiKey, secretKey, webhookUrl, webhookSecret, calendarId, companyId, fromEmail, fromName, createIntegrationConfig, updateIntegrationConfig, navigate, showToast, t]);
                                    ...
                                    {PROVIDER_CATALOG.map((p) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <Paragraph data-size="sm" className={isSelected ? styles.providerNameSelected : styles.providerName} data-selected={isSelected}>
                                                    {p.name}
                                                </Paragraph>
                            ...
                            <div className={styles.cardInner}>
                                ...
                                <Stack direction="vertical" spacing="var(--ds-size-4)">
                                    ...
                                    <Stack direction="vertical" spacing="var(--ds-size-1)">
                                        <Paragraph data-size="sm" className={styles.labelText}>{t('integrationEdit.labelEnvironment')}</Paragraph>
                                        <Select
                                            value={environment}
                        ...
                        <Card data-color="neutral">
                            <div className={styles.cardInner}>
                                ...
                                <Stack direction="vertical" spacing="var(--ds-size-3)">
                                    ...
                                    />
                                    <MetadataRow
                                        label={t('integrationEdit.reviewName')}
                                        value={integrationName || '—'}
                                        size="sm"
                                    />
                                    <MetadataRow
                                    ...
                                    <MetadataRow
                                        ...
                                        value={environment === 'sandbox' ? t('integrationEdit.envSandbox') : t('integrationEdit.envProduction')}
                                        size="sm"
                                    />
                                    {apiKey && (
                                        <MetadataRow
                                    ...
                                    {companyId && (
                                        <MetadataRow
                                            label={t('integrationEdit.reviewOrgNr')}
                                            value={companyId}
                                            size="sm"
                                        />
                                    )}
                ...
                <div className={styles.footerRight}>
                    {isLastStep ? (
                        <Button variant="primary" onClick={handleSaveDraft} disabled={isSaving}>
                            {isSaving ? t('integrationEdit.buttonPublishing') : (isEditMode ? t('integrationEdit.buttonUpdate') : t('integrationEdit.buttonCreate'))}
                        </Button>
                    ) : (
