...
import { Button, Heading, Paragraph, Card, Spinner, Tag, EmptyState, PageContentLayout, PlusIcon } from '@digipicks/ds';
import {
    ...
    <PageContentLayout>
      <div className={styles.page}>
        <div className={styles.header}>
          ...
          <Button data-size="sm" onClick={() => navigate('/broadcasts/compose')}>
          ...
          broadcasts.map((b) => (
            <Card key={b.id} className={styles.broadcastCard}>
              ...
              <div className={styles.broadcastMeta}>
                ...
                <Paragraph data-size="sm">{formatDate(b.sentAt)}</Paragraph>
              </div>
            </Card>
          ))
        )}
    ...
    <PageContentLayout>
      <div className={styles.page}>
        <div className={styles.header}>
          ...
          >
            {unreadOnly ? t('broadcasts.showAll', 'Show All') : t('broadcasts.unreadOnly', 'Unread Only')}
          </Button>
        </div>
        ...
        {broadcasts.length === 0 ? (
          ...
          broadcasts.map((b) => (
            ...
            >
              <Heading data-size="xs">{b.title}</Heading>
              {b.creator && <Paragraph data-size="sm">{b.creator.displayName ?? b.creator.name}</Paragraph>}
              <Paragraph data-size="sm">{b.body}</Paragraph>
              <div className={styles.broadcastMeta}>
                <Tag data-size="sm" data-color={typeColor(b.messageType)}>
                  {typeLabel(b.messageType)}
