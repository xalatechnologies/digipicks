...
import { Button, Heading, Paragraph, Card, Spinner, Textfield, PageContentLayout, EmptyState } from '@digipicks/ds';
import { usePublicTiers, useUpdateTierTrialDays, type SubscriptionTier } from '@digipicks/sdk';
      ...
      <div className={styles.trialConfig}>
        <div className={styles.trialInput}>
          <Textfield
            type="number"
            value={trialDays}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTrialDays(e.target.value)}
            aria-label={t('trialSettings.trialDaysLabel', 'Trial days')}
            data-size="sm"
          />
        </div>
        <Paragraph data-size="sm">{t('trialSettings.days', 'days')}</Paragraph>
        {hasChanged && (
          <Button data-size="sm" onClick={() => onSave(tier._id, inputValue)} disabled={isSaving}>
      ...
      {tiers.length === 0 ? (
        ...
        <Card>
          {tiers.map((tier: SubscriptionTier) => (
            <TierTrialRow key={tier._id} tier={tier} onSave={handleSave} isSaving={isSaving} />
          ))}
        </Card>
      )}
