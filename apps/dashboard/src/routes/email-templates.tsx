...
import {
  useEmailTemplates as useEmailTemplatesQuery,
...
export function EmailTemplatesPage() {
  ...
  const categoryMeta = useMemo(
    (): Record<string, { label: string; statusColor: BadgeColor }> => ({
  ...
  const filteredTemplates = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter((tmpl) => tmpl.name.toLowerCase().includes(q) || tmpl.subject.toLowerCase().includes(q));
    }
    return result;
  ...
  const categoryTabs = useMemo(
    () =>
      categoryOptions.map((opt) => ({
        id: opt.id,
        label:
          opt.id === 'all' ? `${opt.label} (${templates.length})` : `${opt.label} (${categoryCounts[opt.id] ?? 0})`,
      })),
    ...
    () => [
      {
        ...
        render: (row) => (
          <Stack direction="horizontal" spacing="var(--ds-size-3)" align="center">
            <IconBox icon={<MailIcon />} variant={row.isActive ? 'accent' : 'neutral'} size="sm" />
        ...
        render: (row) => (
          <Stack direction="vertical" spacing="0">
            ...
            {row.modifiedBy && (
              <Paragraph
                data-size="xs"
                style={{ color: 'var(--ds-color-neutral-text-subtle)', margin: 0, opacity: 0.7 }}
              >
                {row.modifiedBy}
              </Paragraph>
            )}
    ...
    <PageContentLayout>
      ...
      <Grid columns="repeat(4, 1fr)" gap="var(--ds-size-4)">
        {[
          {
            icon: <MailIcon />,
            label: t('emailTemplates.totalTemplates'),
            value: stats.total,
            variant: 'accent' as const,
          },
          {
            icon: <CheckCircleIcon />,
            label: t('emailTemplates.activeTemplates'),
            value: stats.active,
            variant: 'success' as const,
          },
          {
            icon: <SendIcon />,
            label: t('emailTemplates.totalSent'),
            value: stats.totalSent,
            variant: 'warning' as const,
          },
          {
            icon: <ChartIcon />,
            label: t('emailTemplates.categories'),
      ...
      {/* ─── Category Tabs ─── */}
      <PillTabs tabs={categoryTabs} activeTab={activeView} onTabChange={(id) => setActiveView(id as ActiveView)} />
      ...
      {error ? (
        ...
        <Card data-color="neutral" style={{ padding: 'var(--ds-size-12)' }}>
          <EmptyState
            ...
            description={searchQuery ? t('emailTemplates.noResultsDesc') : t('emailTemplates.noTemplatesDesc')}
            action={
              !searchQuery ? (
                <Button data-size="sm" onClick={handleNewTemplate}>
                  <PlusIcon />
