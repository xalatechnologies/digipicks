...
export function UsersPage() {
  ...
  const activeFilters = useMemo(() => {
    ...
    if (statusFilter)
      f.push({ key: 'status', label: `Status: ${STATUS_MAP[statusFilter as UserStatus]?.label || statusFilter}` });
    ...
    (user: UserRow): Action[] => {
        ...
        actions.push({
          ...
          onClick: async () => {
            const ok = await confirm({
              ...
              description: `Suspender "${user.name}"?`,
              confirmText: 'Suspender',
              variant: 'danger',
            });
            if (ok) {
              try {
                await updateUser({ userId: user.id as any, status: 'suspended' });
                showToast({ title: `${user.name} suspendert`, variant: 'warning' });
              } catch {
      ...
      actions.push({
        ...
        onClick: async () => {
          const ok = await confirm({
            title: 'Slett bruker',
            description: `Slett "${user.name}" permanent?`,
            confirmText: 'Slett',
            variant: 'danger',
          });
          if (ok) {
            try {
              await deleteUserMut({ userId: user.id as any });
              showToast({ title: `${user.name} slettet`, variant: 'success' });
            } catch {
  ...
  const columns: DataTableColumn<UserRow>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        width: '130px',
        render: (row) => (
          <Tag data-size="sm" data-color={(ROLE_COLOR[row.role] ?? 'neutral') as any}>
            {row.role}
          </Tag>
        ),
      },
      ...
      },
      {
        ...
        width: '100px',
        render: (row) => {
          const s = STATUS_MAP[row.status] ?? { color: 'neutral' as const, label: row.status || 'Ukjent' };
          return (
        ...
        render: (row) => (
          <Stack direction="horizontal" spacing="var(--ds-size-1)" onClick={(e) => e.stopPropagation()}>
            ...
            <Button
              ...
              variant="secondary"
              data-size="sm"
              onClick={() => navigate(`/platform/users/${row.id}/edit`)}
            >
              Rediger
            </Button>
            <Button
              ...
              onClick={async () => {
                ...
                if (ok) {
                  try {
                    await deleteUserMut({ userId: row.id as any });
  ...
  return (
    <PageContentLayout>
        ...
        <EmptyState
          ...
          description={
            searchQuery || statusFilter || roleFilter
              ? 'Prov a endre sok eller filtre.'
              : 'Ingen brukere registrert enna.'
          }
          action={
            searchQuery || statusFilter || roleFilter ? (
              <Button
                  ...
                  <Stack direction="vertical" spacing="var(--ds-size-2)" style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <Stack direction="horizontal" justify="between" align="center">
                      ...
                      >
                        ...
                        <Tag data-size="sm" data-color={(ROLE_COLOR[user.role] ?? 'neutral') as any}>
                          {user.role}
                        </Tag>
                      </Stack>
                      <Stack direction="horizontal" spacing="var(--ds-size-2)" onClick={(e) => e.stopPropagation()}>
                        ...
                        {(user.status === 'suspended' || user.status === 'inactive') && (
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                await updateUser({ userId: user.id as any, status: 'active' });
                                showToast({ title: `${user.name} aktivert`, variant: 'success' });
                              } catch {
                    ...
                    <Paragraph data-size="sm" data-color="subtle" style={{ margin: 0 }}>
                      {user.email}
                      {user.tenant !== 'Plattform' ? ` · ${user.tenant}` : ''}
                    </Paragraph>
                  </Stack>
                </Stack>
              </Card>
      ...
      >
        <DrawerSection
          title="Ny bruker"
          description="Brukeren mottar en invitasjon per e-post med lenke for a opprette konto."
        >
          <Stack spacing="var(--ds-size-4)">
            ...
            <Stack direction="vertical" spacing="var(--ds-size-2)">
              ...
              <PillDropdown
                label={
                  { user: 'Bruker', owner: 'Eier (utleier)', admin: 'Admin', super_admin: 'Plattformadmin' }[
                    inviteForm.role
                  ] || 'Velg rolle'
                }
