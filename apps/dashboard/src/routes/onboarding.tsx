...
import { useCreateTenant, useCheckSlugAvailable } from '@digipicks/sdk';
...
export function OnboardingPage() {
  ...
  const canProceedStep0 =
    accountType === 'private' || (accountType === 'organization' && orgVerifyResult?.valid === true);
        ...
        {steps.map((label, i) => (
          <Paragraph
            ...
            style={{
              ...
              color: i === step ? 'var(--ds-color-accent-text-default)' : 'var(--ds-color-neutral-text-subtle)',
            }}
          >
            {i + 1}. {label}
          </Paragraph>
        ...
        {step === 3 && (
          <Stack direction="vertical" spacing="var(--ds-size-4)">
            ...
            >
              ...
              {contactEmail && (
                <Paragraph data-size="sm">{`${t('onboarding.contactEmail', 'Kontakt e-post')}: ${contactEmail}`}</Paragraph>
              )}
              {contactPhone && (
                <Paragraph data-size="sm">{`${t('onboarding.contactPhone', 'Telefonnummer')}: ${contactPhone}`}</Paragraph>
              )}
            ...
            <Stack direction="horizontal" spacing="var(--ds-size-3)" style={{ justifyContent: 'space-between' }}>
              ...
              <Button variant="primary" onClick={handleCreate} disabled={isSubmitting} loading={isSubmitting}>
